...
# experiment = 'target_flex'
experiment = 'align_flex'

# For testing a few trials
# ExpBlocks = ["Practice"]
ExpBlocks = ['flex-align']
# ExpBlocks = ["Testing"]

# ----------- Participant info ----------------

# For clamp and rotation direction
rot_direction = 1  # 1 for forward, -1 for backward
participant = 8
...
dir_path = f"data/{experiment}/p{str(participant)}"
...
# set up file path
file_path = f"data/{experiment}/p{str(participant)}/p{str(participant)}"
...
cursor_size = 0.5
target_size = 1.5
home_range_lower = 1600
home_range_upper = 1800
...
for block in range(len(ExpBlocks)):
    ...
    for i in range(len(condition.trial_num)):
        ...
        while True:
            ...
            win.flip()
            if current_pos[0] < home_range_lower:
