...
def get_xy(task):
    while True:
        vals = task.read(number_of_samples_per_channel = nidaqmx.constants.READ_ALL_AVAILABLE)
        if vals == None:
            continue
        elif not vals == None:
            x_data = vals[0]
...
def contains(small_circ, large_circ):
    d = np.sqrt((small_circ.pos[0] - large_circ.pos[0])*2 + (small_circ.pos[1] - large_circ.pos[1])*2)
